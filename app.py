...
# VIDEO_DIRECTORY =  "/home/subramanian/Videos/HCVid"# change via some options menu
VIDEO_DIRECTORY = "/home/subramanian/Downloads/Actor1"
...
class MenuBar(tk.Menu):
    def __init__(self, parent):
        ...
        menu_file.add_command(label="Refresh Application", command=lambda: resetPage())
        ...
        self.add_cascade(label="Settings", menu=menu_operations)
        ...
        self.add_cascade(label="Other", menu=menu_help)
...
class MyApp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.title("Phyisotherapy Assisstant")
        self.resizable(False, False)
        # self.resizable(0, 0) prevents the app from being resized

        #set app size based on image
        global IMG_HEIGHT, IMG_WIDTH
        IMG_HEIGHT, IMG_WIDTH = getImgSize(THUMBNAIL_DIRECTORY + "/thumbnail" + str(col_counter) + ".png")
        IMG_HEIGHT = IMG_HEIGHT // 2
        IMG_WIDTH = IMG_WIDTH // 2
        self.geometry(str( IMG_WIDTH*3 + IMG_WIDTH//2) + "x" + str(IMG_HEIGHT*2 + IMG_HEIGHT//2)) #fixes the applications size
        self.frames = {}
        # pages = PageOne
        # For multiple frames
        # for F in pages:
...
class GUI(tk.Frame):
    def __init__(self, parent):
        ...
        #set column spaces
        self.main_frame.grid_rowconfigure(0,uniform="rowgroup", weight=2)
        ...
        # self.main_frame.grid_rowconfigure(3, uniform="rowgroup", weight=2)
...
class PageOne(GUI):
    ...
    def craftFrame(self, parent, controller):
        ...
        # frame_label.grid(row=row_counter, column=0, columnspan=4, sticky="nsew")
        #make reset buttn
        ...
        # myscrollbar=tk.Scrollbar(self.main_frame,orient="vertical",command=canvas.yview)
        # canvas.configure(yscrollcommand=myscrollbar.set)
        # myscrollbar.pack(side="right",fill="y")
        # canvas.pack(side="left")
        # canvas.create_window((0,0),window=frame,anchor='nw')
        # frame.bind("<Configure>",scrollConfig)
        # self.main_frame.configure(yscrollcommand=vsb.set)
        self.btnArr = []
        print(self.btnArr)
        time.sleep(1)
        for idx, item in enumerate(sorted(os.listdir(VIDEO_DIRECTORY))):
            # print(item)
            if item.endswith(('.mp4', '.avi', '.webm')):
                ...
                getImgSize(THUMBNAIL_DIRECTORY + "/thumbnail" + str(col_counter) + ".png")
                # create_button(root,vid_name)\
                # print(col_counter)
                img = tk.PhotoImage(file = THUMBNAIL_DIRECTORY + "/thumbnail" + str(col_counter) + ".png")
                # img = Image.open(THUMBNAIL_DIRECTORY + "/thumbnail" + str(col_counter) + ".png")
                ...
                # label = tk.Label()
                btn = tk.Button(self.main_frame,height=IMG_HEIGHT + 10, width=IMG_WIDTH, text = item, image = img, compound = 'top', command = lambda vid_name = item: launch_video(vid_name))
                btn.image = img
                btn.vid_path = col_counter
                btn.vid_name = item
                self.btnArr.append(btn)
                self.btnArr[-1].grid(column=col_counter%3, row=row_counter, sticky="nsew",padx=1, pady=2)
                # btn.grid(column=col_counter%4, row=row_counter, sticky="nsew",padx=1, pady=1)
                col_counter += 1
        ...
        # vsb.grid(row=0, column=1,rowspan=4, sticky='ns')
        # self.main_frame.configure(yscrollcommand=vsb.set)
        
def scrollConfig(canvas,event):
...
def launch_video(vid_name):
    vid_path = VIDEO_DIRECTORY + "/" + vid_name
    print(vid_path)
    # OpenNewWindow()
    cmd = " python3 pta.py --video " + vid_path
    os.system(cmd)
...
class OpenNewWindow(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        frame1 = ttk.LabelFrame(main_frame, text="Thank You")
